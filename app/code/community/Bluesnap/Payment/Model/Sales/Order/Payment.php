...
/**
 * overide authorize function to remove the comment
 * @package     Bluesnap_Payment
 * @license    http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */
class Bluesnap_Payment_Model_Sales_Order_Payment extends Mage_Sales_Model_Order_Payment
{
    ...
    public function capture($invoice)
    {
        ...
        if ($payment = $order->getPayment()) {
            if ($payment->getMethodInstance()->getCode() != 'cse') return parent::capture($invoice);
        }
        if (is_null($invoice)) {
        ...
        if (!$invoice->getIsPaid() && !$this->getIsTransactionPending()) {
            ...
            if ($order->isNominal()) {
                ...
                if ($this->getIsTransactionPending()) $message = $this->_appendTransactionToMessage($transaction, $message);
            }
    ...
    protected function _authorize($isOnline, $amount)
    {
        $order = $this->getOrder();
        if ($payment = $order->getPayment()) {
            if ($payment->getMethodInstance()->getCode() != 'cse') return parent::_authorize($isOnline, $amount);
        }
